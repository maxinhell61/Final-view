...
@admin_bp.route("/toggle_active_all", methods=["PUT"])
@jwt_required()
def toggle_all_users_active():
    current_user_id = get_jwt_identity()
    ...
    users = User.query.all()
    for user in users:
        user.isActive = not user.isActive  # toggle
    db.session.commit()

    return jsonify({"message": "Toggled isActive status for all users."}), 200



@admin_bp.route("/toggle_active/<int:user_id>", methods=["PUT"])
@jwt_required()
def toggle_user_active(user_id):
    ...
    return jsonify({
        "message": f"User {user.id} isActive status toggled to {user.isActive}."
    }), 200
